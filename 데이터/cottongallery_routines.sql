...
/*!50001 VIEW `sample` AS select `t`.`fabric_id` AS `fabric_id`,`f`.`form` AS `form`,`f`.`size` AS `size` from (`fabric` `t` join `form` `f`) where (`f`.`form` <> 'curtain') order by `t`.`fabric_id`,`t`.`fabric_id` */;
